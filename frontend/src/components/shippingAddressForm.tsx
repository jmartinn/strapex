...
export default function ShippingAddressForm({
  ...
  const handleInternalNextClick = () => {
    if (!shippingAddress?.name || !shippingAddress?.surname || !shippingAddress?.address || !shippingAddress?.city || !shippingAddress?.state || !shippingAddress?.country || !shippingAddress?.postcode) {
  ...
  return (
    <form className="space-y-4">
      ...
      <Button type="button" onClick={handleInternalNextClick} className="w-full bg-neutral-800 text-white px-6 py-3 rounded font-semibold hover:bg-gray-600" size="3">
        Next
      </Button>
    </form>
  );
