...
async function main() {
  try {
    ...
    const eventSelector = FieldElement.fromBigInt(
      BigInt("0x9149d2123147c5f43d258257fef0b7b969db78269369ebcf5ebb9eef8592f2")
    );
    ...
    function createFilter() {
      let filter = Filter.create().withHeader({ weak: true });
      //Listen for the event from the tracked addresses
      trackedAddresses.forEach((address) => {
        filter = filter.addEvent((ev) =>
          ev.withFromAddress(address).withKeys([eventSelector])
        );
      });
      ...
      filter = filter.withStateUpdate((stateUpdateFilter) =>
        stateUpdateFilter.addDeployedContract((deployedContractFilter) => {
          return deployedContractFilter.withClassHash(classHash);
        })
      );
          ...
          for (const data of message.data.data) {
            ...
            for (const event of events) {
              ...
              if (event.event && event.receipt) {
                ...
                const from = event?.event?.keys
                  ? FieldElement.toHex(event.event.keys[1])
                  : null;
                const amount = event?.event?.keys
                  ? FieldElement.toBigInt(event.event.keys[2])
                  : null;
                const id = event?.event?.keys
                  ? FieldElement.toBigInt(event.event.keys[3])
                  : null;
                ...
                const token = event?.event?.data
                  ? FieldElement.toHex(event.event.data[0])
                  : null;
                ...
                try {
                  ...
                  const eventData = {
                    ...
                    transactionHash: event.transaction?.meta?.hash
                      ? FieldElement.toHex(event.transaction.meta.hash)
                      : null,
                    from,
                    amount,
                    depositId: id,
                    toAddress: event?.event?.fromAddress
                      ? FieldElement.toHex(event.event.fromAddress)
                      : null,
                    token,
                  ...
                  if (!existingEvent) {
                    ...
                    console.log(
                      "Event with the same transaction hash already exists. Skipping insertion."
                    );
                  }
                } catch (error) {
            ...
            if (
              ...
              for (const deployedContract of stateUpdate.stateDiff
                .deployedContracts) {
              ...
              for (const deployedContract of stateUpdate.stateDiff
                ...
                const deployedClassHash = FieldElement.toHex(
