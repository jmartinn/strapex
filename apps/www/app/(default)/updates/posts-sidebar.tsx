...
export default function PostsSidebar({
  ...
  return (
    <aside className="mb-12 md:mb-0 md:mr-8 md:w-[276px] md:shrink-0 md:p-4">
      <div className="mb-3 text-xs font-semibold uppercase tracking-normal text-gray-500/80">
        Select category
      </div>
      <ul className="space-y-0.5">
        {categories.map((category, index) => {
          return (
            <li key={index}>
              <a
                className={`flex h-9 items-center space-x-4 rounded-lg px-4 text-sm font-medium text-gray-800 transition-colors dark:text-gray-200 ${selectedCategory === category ? "bg-gradient-to-tr from-white/70 to-white/50 shadow shadow-black/5 dark:bg-gradient-to-b dark:from-gray-700/50 dark:to-gray-700/40" : "hover:bg-indigo-50/50 dark:hover:bg-gray-700/20"}`}
                href="#0"
                onClick={(e) => {
                  e.preventDefault();
